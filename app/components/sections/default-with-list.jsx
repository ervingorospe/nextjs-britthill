...
export async function DefaultWithList({ data, sectionCount }) {
  const { fields } = data
  ...
  return (
    subCollection?.map(item => (
      <MotionVariant variants={fadeInFromBottom} className="flex flex-col gap-y-3 border-l border-white/30 pl-6" key={item.name}>
        <dt className="order-first text-2xl font-semibold tracking-tight">{item.name}</dt>
        <dd className="text-sm leading-6">{_.get(item, 'fields.subtitle')}</dd>
      </MotionVariant>
    ))
  )
}
