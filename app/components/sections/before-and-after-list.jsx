...
// components
import { Title, Body, Button, BeforeAndAfter } from '@/components/shared'
// motion
...
export async function BeforeAndAfterList({ data, sectionCount }) {
  const { fields } = data
      ...
      <Container width={width} margin="section" className="relative z-1">
...

        {
          subCollection.length > 0 && (
            <Motion className="mx-auto grid lg:grid-cols-2 gap-8">
              {
                subCollection?.map(image => (
                  <BeforeAndAfter key={image.id} data={image}/>
                ))
              }
            </Motion>
          )
