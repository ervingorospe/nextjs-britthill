...
export async function LocalSeoHeadline({ data, sectionCount }) {
  const { fields } = data
      ...
      <Container width={width} margin="section" className="relative z-1">
        <Motion className="grid mx-auto text-center justify-center max-w-5xl">
          ...
          <Title
            ...
            size={subtitle}
            variants={fadeInFromBottom}
            className={`mt-6 text-white text-lg lg:text-xl ${_.get(fields, 'subtitleClass') ?? _.get(fields, 'subtitleClass')}`}
          />

          {
            fields.image && (
              <MotionVariant className="relative mt-4" variants={fadeInFromBottom}>
                <ImageHolder
                  image={fields.image} 
                  className={{
                    figure: "relative z-10",
                    image: "shadow-md w-full"
                  }}
                />
