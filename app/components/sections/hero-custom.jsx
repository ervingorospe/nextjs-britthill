...
// function
import { getHomepageImageSlider } from '@/function/page'
...
export async function HeroCustom({ data, sectionCount }) {
  ...
  const sliderImages = await getHomepageImageSlider()

  return (
    <Section className="relative w-full lg:h-[100vh]" bg={bg}>
        ...
        <div className="block lg:absolute h-[30vh] md:h-[50vh] lg:h-full w-full lg:inset-0">
          {
            _.get(fields, 'video') ? (
              <figure className="h-full w-full">
                <video className="w-full h-full object-cover object-center" playsInline autoPlay muted loop>
                  <source src={_.get(fields, 'video.videoUrl')} type="video/mp4"/>
                </video>
              </figure>
            ) :
            (
